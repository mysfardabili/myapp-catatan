 ...
 */
import { archive as api_notes_archive_archive } from "~backend/notes/archive";
import { create as api_notes_create_create } from "~backend/notes/create";
import { remove as api_notes_delete_remove } from "~backend/notes/delete";
import { get as api_notes_get_get } from "~backend/notes/get";
import { list as api_notes_list_list } from "~backend/notes/list";
import { pin as api_notes_pin_pin } from "~backend/notes/pin";
...
export namespace notes {
    ...
    export class ServiceClient {
        ...
        public async archive(params: RequestType<typeof api_notes_archive_archive>): Promise<ResponseType<typeof api_notes_archive_archive>> {
            ...
            const resp = await this.baseClient.callTypedAPI(`/notes/${encodeURIComponent(params.id)}/archive`, {method: "PUT", body: JSON.stringify(body)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_notes_archive_archive>
        }
        ...
        public async pin(params: RequestType<typeof api_notes_pin_pin>): Promise<ResponseType<typeof api_notes_pin_pin>> {
            // Construct the body with only the fields which we want encoded within the body (excluding query string or header fields)
            const body: Record<string, any> = {
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/notes/${encodeURIComponent(params.id)}/pin`, {method: "PUT", body: JSON.stringify(body)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_notes_pin_pin>
        }
